...
if length(elec)>1
    ...
    if length(unique(clu(goodCluIx)))>1
        ...
        if doMerge
            ...
            % Here is a light version of 'mergeclu_slow' that is used only
            % to reorder the clusters by similarity for fast visualisation
            % clusters to be merge in Klusters.
