...
%     Properties    Values
%    -------------------------------------------------------------------------
%     'spacing'     vertical spacing between variables (default = 5000)
%    =========================================================================
%
%  SEE
%
...
n = size(W,2);
for i = 1:n,
	%plot(spacing*(n-i)+permute(squeeze(W(:,i,:)),[2 1]),v{:});
	plot(spacing*(n-i)+permute(squeeze(W(:,i,:)),[2 1]));
end
...
%  for i = 1:8,
%  	plot(spacing*(n-i)+permute(squeeze(W(:,i,:)),[2 1]),v{:});
%  	plot(permute(squeeze(W(:,i,:)),[2 1]));
%  	%plot(spacing*(n-i)+squeeze(W(:,i,:)));
%  end
...
%  for i=1:size(W,1)
%    for j=1:8
%        plot(squeeze(W(i,j,:))+spacing*(j-1))
%    end
%  end
